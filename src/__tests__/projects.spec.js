...
describe("Projects", () => {
  it("should be able to list the projects", async () => {
    ...
    expect(response.body.repositories).toMatchObject(
      [
        {
  ...
  it("should be able to delete the repository", async () => {
    ...
    const repositoriesList = await request(app).get("/repositories");

    const { repositories } = repositoriesList.body;    

    const repository = repositories.find(r => r.id === response.body.id);
